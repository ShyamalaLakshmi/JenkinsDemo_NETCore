...
{
    [TestClass]
    public class UsersControllerPerformanceTests
    {
            ...
            for (var i = 0; i < 100; i++)
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (!tokenResponse.StatusCode.Equals(HttpStatusCode.BadRequest))
                    {
                        ...
                        var request = new HttpRequestMessage(new HttpMethod("GET"), "http://localhost:8091/api/Users");
            ...
            for (var i = 0; i < 100; i++)
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (!tokenResponse.StatusCode.Equals(HttpStatusCode.BadRequest))
                    {
                        ...
                        var request = new HttpRequestMessage(new HttpMethod("GET"), "http://localhost:8091/api/Users/Pmo");
                        request.Headers.Add("Authorization", $"Bearer {token}");
