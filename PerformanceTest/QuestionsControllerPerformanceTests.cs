...
{
    [TestClass]
    public class QuestionsControllerPerformanceTests
    {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (!tokenResponse.StatusCode.Equals(HttpStatusCode.BadRequest))
                    {
                        ...
                        var request = new HttpRequestMessage(new HttpMethod("GET"),
                            "http://localhost:8091/api/Questions");
            ...
            for (var i = 0; i < 100; i++)
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (!tokenResponse.StatusCode.Equals(HttpStatusCode.BadRequest))
                    {
                        ...
                        var request = new HttpRequestMessage(new HttpMethod("GET"),
                            "http://localhost:8091/api/Questions/ee2b83be-91db-4de5-8122-35a9c9195975");
                        request.Headers.Add("Authorization", $"Bearer {token}");
