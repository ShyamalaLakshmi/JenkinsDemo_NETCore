...
{
    [TestClass]
    public class EventsControllerPerformanceTests
    {
            ...
            for (var i = 0; i < 100; i++)
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (!tokenResponse.StatusCode.Equals(HttpStatusCode.BadRequest))
                    {
                        ...
                        var request = new HttpRequestMessage(new HttpMethod("GET"), "http://localhost:8091/api/Events");
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (!tokenResponse.StatusCode.Equals(HttpStatusCode.BadRequest))
                    {
                        ...
                        var request = new HttpRequestMessage(new HttpMethod("GET"),
                            "http://localhost:8091/api/Events/true");
                        request.Headers.Add("Authorization", $"Bearer {token}");
            ...
            for (var i = 0; i < 100; i++)
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (!tokenResponse.StatusCode.Equals(HttpStatusCode.BadRequest))
                    {
                        ...
                        var request = new HttpRequestMessage(new HttpMethod("GET"),
                            "http://localhost:8091/api/Events/B6FC0079-8FE2-468D-80E0-10F927DD69A9");
                        request.Headers.Add("Authorization", $"Bearer {token}");
